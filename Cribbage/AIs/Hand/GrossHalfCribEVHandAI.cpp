#include "GrossHalfCribEVHandAI.h"
...
std::vector<Card> GrossHalfCribEVHandAI::select(const std::vector<Card>& dealt, bool isMyCrib)
{
    ...
    for (int i = 0; i < dealt.size() - 1; i++)          // run through all possible throws
    {
        for (int j = i + 1; j < dealt.size(); j++)
        {
            ...
            float ev = grossHalfCribExpectedValue(toKeep, toThrow, isMyCrib);
            if (ev > highestEV)
            {
                highestEV = ev;
                highestCards = toKeep;
...
float GrossHalfCribEVHandAI::grossHalfCribExpectedValue(const std::vector<Card>& toKeep, const std::vector<Card>& toThrow, bool isMyCrib)
{
...
int GrossHalfCribEVHandAI::halfCribScore(const std::vector<Card>& toThrow, const Card& cut)
{
    int score = 0;
